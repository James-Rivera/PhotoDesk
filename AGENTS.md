...
- 2026-08-21 — Added branch-local offline hosting for shop workstations. A locally bound Next.js production server now supports a separate PBKDF2-protected staff login and signed 12-hour session without Supabase or internet, while preserving local image loading, exact layouts/PDFs, downloads, and printing. Cloud-only Library, maintenance, and rembg destinations are visibly unavailable and cannot be opened in this mode. Added safe setup/start scripts, a cold-start runbook, and local-auth tests; 52 tests, typecheck, lint, production build, route protection, and a real local login pass.
- 2026-08-21 — Expanded offline hosting into a one-run Windows branch server shared across the Private local subnet. The installer downloads and SHA-256-verifies a pinned portable Node.js runtime, configures/builds PhotoDesk, registers a hidden SYSTEM startup task, opens TCP 3210 only for Private/LocalSubnet clients, starts the service, and reports bookmarkable LAN URLs; its uninstaller removes the task, firewall rule, and shortcut without deleting credentials. Added in-memory local-login throttling and updated Print Helper 0.4.2 to pair with RFC1918 PhotoDesk origins while remaining loopback-bound and rejecting public HTTP origins.
