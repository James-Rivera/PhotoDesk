...
internal sealed class HelperContext : ApplicationContext
{
    ...
    private async Task OpenPrintDialogAsync(HttpListenerContext context, string? origin)
    {
        ...
        if (context.Request.ContentLength64 is < 1 or > MaxPrintSheetBytes)
        {
            await WriteError(context.Response, 413, "The print sheet is empty or larger than 30 MB.");
            return;
        }

        if (!string.Equals(context.Request.ContentType, "image/png", StringComparison.OrdinalIgnoreCase))
        {
            await WriteError(context.Response, 415, "CJNET Print Helper accepts its print-ready PNG sheet only.");
            return;
        }

        var sheetBytes = new byte[(int)context.Request.ContentLength64];
        await context.Request.InputStream.ReadExactlyAsync(sheetBytes);
        try
        {
            PrintSheetImage.Validate(sheetBytes);
        }
        catch (InvalidDataException error)
        {
            await WriteError(context.Response, 415, error.Message);
            return;
        }

        var path = Path.Combine(tempDirectory, $"CJNET-{Guid.NewGuid():N}.png");
        await File.WriteAllBytesAsync(path, sheetBytes);
        BeginInvokeOnUi(() => ShowPrintWindow(path));
        await WriteJson(context.Response, new { ok = true, message = "CJNET Windows print window opened." });
    }
    ...
    private void ShowPrintWindow(string path)
    {
        ...
        {
            MessageBox.Show("Finish or close the current print window first.", "CJNET Print Helper", MessageBoxButtons.OK, MessageBoxIcon.Information);
            TryDelete(path);
            return;
        }
    ...
    private void BeginInvokeOnUi(Action action) => dispatcher.BeginInvoke(action);
