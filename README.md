...
For the normal staff workflow, run `CJNET-Print-Helper-Setup.exe` once per workstation as described in [CJNET Windows Print Helper](docs/WINDOWS-PRINT-HELPER.md). Staff do not need VS Code or developer tools. PhotoDesk then sends the in-memory PDF to localhost and opens the native Windows dialog, where staff can select any existing printer and open its real Epson Preferences screen. Browser printing and PDF download remain fallbacks.
