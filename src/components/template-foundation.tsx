...
export function TemplateFoundation() {
  ...
  async function makePdf() {
    ...
    return generatePhotoSheetPdf({
      ...
      sources: {
        ...
        small: { image: smallSource.image, crop: crops.small },
      },
      borders,
      borderColor,
      borderThickness,
  ...
  async function downloadPdf() {
    ...
    try {
      ...
      anchor.href = url; anchor.download = `CJNET-${activeDefinition?.name ?? "Custom"}-${new Date().toISOString().slice(0, 10)}.pdf`; anchor.click();
      window.setTimeout(() => URL.revokeObjectURL(url), 30_000);
      setNotice("Print-ready A4 PDF downloaded.");
...
function Message({ kind, text }: { kind: "ok" | "error"; text: string }) { return <div className={`mt-2.5 flex items-start gap-2 rounded-lg border p-2.5 text-[12px] ${kind === "ok" ? "border-[#cbe3c6] bg-[#eef6ec] text-[#255c2f]" : "border-[#efc0b2] bg-[#fdf0ec] text-[#8c2410]"}`}>{kind === "ok" ? <Check size={15} className="shrink-0" /> : <AlertTriangle size={15} className="shrink-0" />}<span>{text}</span></div>; }
